...
**After `/cairn:build`:** a 15-minute interview scaffolds a study coach with its own memory
...
Cairn is a Claude Code plugin that scaffolds long-lived personal agentic systems: a kernel
...
Then, in an empty directory: `/cairn:build`
...
- Uninstall the plugin: instances keep working (runtime is instance-local). Delete an
  instance directory: Cairn retains nothing about it.
...
2. **Typed-lapse abandonment model** — lapses are forgot/upkeep/skipped/suspended, recoverable,
   and never guilt-framed; upkeep lapses are treated as Cairn bugs. /conclude is a success state.
