...
6. **`/cairn:list` — the portfolio view (new plugin skill).** Reads the registry, then reads *live* per entry from the instance's own files. Status derivation (in precedence order):
   ...
   A naive "last event" read is wrong by construction — `session_end.py` appends a `session phase=end` event after any suspend/conclude in the same session, so the tail is always a session event. Plus per entry: `Last reconciled` stamp from `state/HOT.md`, north-star name from `manifest.json`. Renders one table: name · purpose · status · last session · last reconciled (the stamp itself is the staleness signal — no separate flag column shipped). A path that no longer resolves to a manifest shows as `missing — moved or deleted?` and is removed only on user confirmation (the user is the gate; no auto-pruning). P13 constraint: the list reports status, never guilt — no "you've neglected X for N days" framing, ever.
...
- **Meta-review across instances** — a governor-style pass over all systems (shared patterns, conflicting commitments, decay). The "hub instance" growth path; design it only when someone runs enough instances to want it. **Design shape recorded (R10, 2026-07-24; still BET — do not build yet):** the nearest validated analog is fleet/multi-tenant noisy-neighbor observability (US Patent 11,928,518), which prescribes a three-part pattern — (1) *aggregate* multiple pre-existing per-instance telemetry streams into one repository (not one bespoke stream); (2) *correlate* each instance's own patterns into a single per-instance "correlated usage pattern" rather than comparing raw per-instance thresholds; (3) run cross-instance *impact analysis* (unsupervised anomaly detection, not fixed rule thresholds) to separate systemic from isolated failure. This maps directly onto the portfolio signals a hub would want (shared lapse patterns, portfolio-level metrics, systemic-vs-isolated separation). But the entire basis is one patent describing an architecture with no measured efficacy, and the open question is explicit: does correlate-then-cross-impact actually beat simple portfolio aggregates for an *agentic* fleet? Unknown without an A/B on real registry data nobody has yet — so this settles the *shape*, not the go/no-go. When a user runs enough instances to want it, build this design and measure it against a plain aggregate baseline. (Ledger: docs/superpowers/specs/2026-07-24-r10-gaps-ledger-closeout-design.md, Decision 5.)
