# Cairn v1 Implementation Plan

**Executed — shipped as 0.1.0 (c206f8b)**

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build the Cairn Claude Code plugin — kernel runtime (hooks + validator + telemetry), instance templates, scaffolder, and the build/review/upgrade skills — per the approved spec at `docs/superpowers/specs/2026-07-19-cairn-design.md`.
...
cairn/
  ...
  skills/
    build/SKILL.md                     # /cairn:build interview protocol
    build/scaffold.py                  # deterministic instance generator
    review/SKILL.md                    # /cairn:review protocol (memory + system lanes)
    upgrade/SKILL.md                   # /cairn:upgrade protocol
    upgrade/merge.py                   # managed-by-cairn header merge
  templates/
    hooks/cairn_lib.py                  # shared: find_root, manifest, event append
    hooks/cairn_event.py                # CLI event writer (used by commands too)
...
Naming locked: instance root marker is `manifest.json` containing key `cairn_version`. Sentinel is `.cairn/review-in-progress`. Events file `telemetry/events.jsonl`; archive `state/archive.jsonl`; hot snapshot `state/HOT.md` with a line `Last reconciled: YYYY-MM-DD`.
...
`marketplace.json` (self-hosted single-plugin marketplace; install: `/plugin marketplace add veer-sanyal/cairn`):
```json
{
  "name": "cairn-marketplace",
  "owner": { "name": "Veer", "url": "https://github.com/veer-sanyal" },
  "plugins": [
    { "name": "cairn", "source": "./", "description": "Kernel + builder for personal agentic systems" }
  ]
}
```
...
### Task 2: `cairn_lib.py` + `cairn_event.py` (telemetry writer)

**Files:**
- Create: `templates/hooks/cairn_lib.py`, `templates/hooks/cairn_event.py`
...
- [ ] **Step 3: Implement `templates/hooks/cairn_lib.py`:**
...
`templates/hooks/cairn_event.py`:
...
- [ ] **Step 2: Write test seeding helper** `tests/cairn_helpers_for_tests.py`:
...
## How this instance works (cairn kernel)
- **Boot:** the session banner shows staleness/gaps/validator state. Reconcile before working.
- **State ownership (one fact, one owner):** current snapshot → `state/HOT.md` (keep lean; update `Last reconciled:` when you sync it). Topic files → `state/working/` (Edit for targeted updates; wholesale rewrites happen only in /cairn:review). History → `state/archive.jsonl` (append-only, via `python3 .claude/hooks/cairn_event.py` or /log).
...
- [ ] **Step 5: Commit** — `git commit -am "feat: /cairn:build skill"`

---

### Task 12: `/cairn:review` skill
...
- [ ] **Step 3: Commit** — `git commit -am "feat: /cairn:review skill"`

---

### Task 13: `/cairn:upgrade` skill + `merge.py`
...
the new version lands as <installed>.cairn-new instead."""
...
1. Read instance manifest cairn_version and the plugin's .claude-plugin/plugin.json version.
...
3. Hook scripts are plugin-owned and never user-edited: copy every file from the plugin's
   ...
   (substitute {{cairn_version}} = NEW version, {{intents}} from manifest.json) into a temp
   dir, then run merge.py <rendered-new> <installed-command> per file. merge.py rules:
   identical → no-op; unmanaged → untouched; managed+different → *.cairn-new alongside
   (stale .cairn-new replaced, latest wins), user's file never overwritten; every action
...
- [ ] **Step 1: Self-check against spec** — walk the spec's §-anchors and confirm each is implemented or explicitly deferred; the known deferrals are: recall-count demotion (BET, recency-only shipped), upstream `/cairn:contribute` (cut), Windows (declared unsupported). No other silent gaps allowed.
...
**NOT in this plan (post-v0.1.0 validation, per spec):** dogfood migration of Veer PM onto the kernel, one greenfield `/cairn:build` run, a telemetry-meaningful soak period, one real review cycle — then GitHub publish. These need real usage time, not implementation tasks.
...
- **Type consistency:** event field names (`type/ts/phase/session_id/cause/outcome/intent`) consistent across cairn_lib, session_start/end, tests, log.md, review skill. Sentinel path `.cairn/review-in-progress` consistent across guard_files, validate, review skill. Build-config keys consistent between Task 9 CFG, scaffold.py, and build SKILL.md.
```
