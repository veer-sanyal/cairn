...
## 25. Objectives before metrics: surface the full objective set, then measure
Perishability: durable · Verified: 2026-07 · Round: R11

**[VERIFIED]** People systematically omit about HALF their own objectives when generating them unaided, and a structured re-prompt recovers ~62% more — rated just as important as the ones named first. Bond, Carlson & Keeney ("Improving the Generation of Decision Objectives," Decision Analysis 7(3) 2010, INFORMS, DAS Best-Publication award; underlying Management Science 2008): N=20 doctoral students choosing a dissertation topic listed 7.25 objectives unaided, added 4.5 more (62%) after a short named-category prompt (t(19)=4.75, p<.01); the added objectives rated 7.3 vs 7.7 importance (n.s.) — just as important. The omission was studied on REAL single-person personal decisions (dissertation, MBA choice), so it transfers to a solo self-builder — the one R11 sub-area that cleanly does. Caveat (authors' own Study 2): part of the gain is the second attempt, so credit STRUCTURED RE-PROMPTING, not category-naming magic. (R11, 3-0.)

**[VERIFIED]** Fundamental vs. means objectives (Keeney, value-focused thinking): separate what you ultimately want (fundamental) from the levers that get you there (means) BEFORE choosing what to measure. This maps directly onto P12 — fundamental objectives → the north star, means objectives → the input levers — so an objectives pass FEEDS the metric contract rather than duplicating it.

**[VERIFIED — direction; the continuity thesis carried one 2-1]** Framing determines solution quality, but it is CONTINUOUS and bidirectional, not a discrete phase that precedes the work. Problem-solution co-evolution: Dorst & Cross 2001 (Design Studies, n=9 solo designers), replicated by Wiltschnig/Christensen/Ball 2013 (63 episodes, distributed throughout, 75% requirement→solution / 19% reverse) and Parsons & Shukla (IEEE VIS 2025, n=11) — "framing is not a preliminary step but a continuous activity." Consequence: make the objectives pass a SHORT, RE-ENTERABLE move the metric contract can loop back into, never a locked upfront "problem-definition stage."

**[THIN / REFUTED — do not import the machinery]** The loud PM-consultant frameworks do not earn their volume. Jobs-to-be-Done / Outcome-Driven Innovation (Ulwick) is sole-inventor patent IP whose "86% success / 5x more predictable" figures are unsubstantiated vendor self-report (0-3), and its defining mechanic — an importance/satisfaction survey of a statistically-valid market sample (180–3,000 respondents) run through the Opportunity Algorithm — is undefined at n=1 and does NOT transfer to a solo builder (market-scoped; refuted). Torres's opportunity-solution-tree is a practitioner visualization aid with no method-level validation (THIN); its one sound idea, outcome-over-output, is already what P12's north-star/guardrail contract encodes. Borrow at most the desired-outcome-statement sentence shape (direction + metric + object + context) as an optional phrasing heuristic — no evidentiary weight, and drop it under YAGNI if it just repackages P12/P18.

**Design implications:**
- The builder runs a lightweight objectives pass (build Stage 2.4) AFTER the user authors the goal (P14) and before the metric contract (P12): prompt across a small fixed set of named categories for what to MAXIMIZE / MINIMIZE / AVOID, split fundamental from means objectives, and ask once "what's the real problem, and who or what does it serve." The user still authors — you prompt, they answer; you never write their objectives.
- The pass is short and re-enterable, not a gate: Stage 3 can loop back into it as the contract takes shape.
- The framing PRINCIPLE is VERIFIED; the specific category set for Cairn's interview is a design [BET] (Keeney used five life-domain categories for a different decision) — tune it from telemetry, and never inflate the step into market research, personas, or a JTBD survey a solo builder has no population to feed.
...
1 objectives-framing round, 171 agent calls, 6 angles, 3-vote adversarial verification, July 24 2026:
R11 PM/problem-framing (45 claims verified / 30 confirmed / 15 killed, research-round11-pm-problem-framing.json) — grounded the new P25 objectives pass in decision science (Keeney/Bond, VERIFIED, clean n=1 transfer) and design-cognition co-evolution (VERIFIED, make it revisable), while refuting JTBD/ODI and opportunity-solution-tree machinery as THIN and market-scoped (undefined at n=1). The framing principle is VERIFIED; the specific build-step category set is a BET.
